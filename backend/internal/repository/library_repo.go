...
func (r *LibraryRepo) Get(ctx context.Context, id int64) (*models.LibraryItem, error) {
	row := r.db.QueryRowContext(ctx, librarySelectColumns+` WHERE id = ?`, id)
	return scanLibraryItem(row)
}

func (r *LibraryRepo) List(ctx context.Context) ([]models.LibraryItem, error) {
	rows, err := r.db.QueryContext(ctx, librarySelectColumns+` ORDER BY downloaded_at DESC`)
	if err != nil {
...
const librarySelectColumns = `
	SELECT id, download_id, title, filename, path, collection_id, folder, original_url, video_id,
	       uploader, duration, resolution, thumbnail, description, downloaded_at, status, file_size_bytes
	FROM library`
...
func scanLibraryItem(row rowScanner) (*models.LibraryItem, error) {
	...
	err := row.Scan(
		&item.ID, &item.DownloadID, &item.Title, &item.Filename, &item.Path, &item.CollectionID, &item.Folder,
